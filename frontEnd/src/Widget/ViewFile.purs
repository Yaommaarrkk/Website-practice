...
type State
  = { input :: String, fileName :: String, fileContent :: String }
  ...
  ClickButton -> do
    ...
    m_respond <-
      H.liftAff $ AX.request
        $ AX.defaultRequest
            { url = "http://127.0.0.1:666/api/file/view/" <> old_st.input
            , method = Left GET
            , responseFormat = AXRF.string -- 回傳內容用string格式解析
            , headers =
              [ -- Accept 告訴後端我想要什麼類型的回應 可以多個排優先級 -- 但後端也可能亂回 通常還需看respond的Content-Type 所以responseFormat通常要設string
    ...
    case m_respond of
      ...
      Left errMsg -> do -- 失敗
        let
          message = "error: " <> printError errMsg
